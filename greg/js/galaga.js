...
var player;
var clonePlayer;
var bullets = []; //bullets fired by player
var barrierBullets = [];
var badGuys = []; //the bad guys
...
var luckyLife = DEBUG ? 1 : 0;
var LUCKY_LIFE_LIMIT = 50;
var KEYBOARD_LISTENERS = true;
...
function selectSpider(){
	var type = null;
	var markedItems = [];
	var maxLen = badGuys.length;

    do {
        var idx = Math.floor(Math.random() * 100) % maxLen
        markedItems[idx] = 1;
        if(badGuys[idx].type){
            type = badGuys[idx].cusType();
        }
    } while(type != 'bad1' && markedItems.length < maxLen);
            ...
            jQuery.each(badGuy.suriArr, function(index, suri){
                try{
                    ...
                    if(suri.right()<0 || suri.left()>400 || suri.top() > 400){
                       removeSuriList.unshift(index);
                    }else{
//                        console.log("suri draw->"+index+"["+suri.left()+","+suri.top()+"]");
                        GALAGA_CONTEXT.drawImage(suri.img, suri.left(), suri.top(), 15, 15);    
                    }
                }catch(e){
                    console.log("suri error->" + suri + " (" + e.message + ")");
                }
            });

			for(var idx in removeSuriList) {
				try {
					removeSuriList.splice(idx, 1);
					if (DEBUG) {
						console.log("suri removed->" + idx);
					}
				} catch(e) {
					console.log("suri removal error->" + idx + " (" + e.message + ")");
				}
			}
			delete removeSuriList;
	}
});
    //Barrier bullets
